...
class File:
    ...
    Variables:
        ...
        file: Path -- Path object created from the name passed in.
              Actually needs to be called path for some reason ...?
    Methods:
        [property]
...
class RealClearPolitics(Site):
        ...
        for rows in table.find_all("tr"):
            ...
            if (len(columns) == 7) and ("Average" not in columns[0].text):
                poll = Poll(
                    Poll=columns[0].text,
                    Date=columns[1].text,
                    Sample=columns[2].text,
                    Biden=float(columns[3].text),
                    Sanders=float(columns[4].text),
                    # Some dodgy data in Gabbard
                    Gabbard=float(columns[5].text.replace("-", "0")),
    ...
    def stats(self, loc: int = 0):
        ...
        print_str = f"\nRealClearPolitics\n"
        print_str += "=================\n"
        print_str += f"   Biden: {biden:.1f}\n"
        print_str += f" Sanders: {sanders:.1f}\n"
        print_str += f" Gabbard: {gabbard:.1f}\n"
...
class NYTimes(Site):
    ...
    def stats(self, loc: int = 0):
        ...
        for poll in self.polls(loc):
            print_str += f"{poll.Candidate.rjust(33)}\n"
            print_str += f"---------------------------------\n"
            print_str += f"National Polling Average: {poll.Average}\n"
            print_str += f"       Pledged Delegates: {poll.Delegates}\n"
            print_str += f"Individual Contributions: {poll.Contributions}\n"
            print_str += f"    Weekly News Coverage: {poll.Coverage}\n"
