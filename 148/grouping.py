...
def group_cars_by_manufacturer(cars):
    ...
    """
    cars_grouped_by_company = groupby(sorted(cars, key=lambda x: x[0]), lambda x: x[0])

    for company, cars in cars_grouped_by_company:
        print(company.upper())
        for _, car in sorted(cars, key=lambda y: y[1]):
            print(f"- {car}")
        print()
