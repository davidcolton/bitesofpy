...
def get_dates(dates):
    ...
    # First count the formats and store the formats in a list
    format_counter = Counter()
    for d in dates:
        for df in _maybe_DateFormats(d):
            format_counter[df] += 1
    top_3_formats = format_counter.most_common(3)
    if (top_3_formats[0][0] == DateFormat.NONPARSABLE) or (
        top_3_formats[0][1] == top_3_formats[1][1] == top_3_formats[2][1]
    ):
        raise InfDateFmtError

    top_format_value = top_3_formats[0][0].value

    formatted_dates = []
    for date in dates:
        try:
            newdate = datetime.strptime(
                date, DateFormat.get_d_parse_formats(top_format_value)
            )
            formatted_dates.append(datetime.strftime(newdate, "%Y-%m-%d"))
        except ValueError:
