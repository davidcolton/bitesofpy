...
def calculate_score(scores):
    ...
    """
    if not all([n in DICE_VALUES for n in scores]):
        raise ValueError

    return sum([n for n in scores if n >= MIN_SCORE])
...
def get_winner(players):
    """Given a list of Player namedtuples return the player
       ...
       output:
         Player(name='player 3', scores=[4, 5, 1, 2])
    """
    if not all(len(player.scores) == len(players[0].scores) for player in players):
        raise ValueError
