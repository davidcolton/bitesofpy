...
def operas_both_at_premiere(guest, composer):
    ...
    """
    # Validate the input:
    if not set([guest, composer]) <= set(composers.keys()):
        raise ValueError

    # List of the composers operas
    composer_operas = [opera for opera in operas if opera.author == composer]

    # Check if guest could have attended
    ...
    for opera in composer_operas:
        opera_date = _get_date(opera.date)
        if (guest_born < opera_date < guest_died) and (opera_date < composer_died):
            yield opera.play
