...
def count_n_repetitions(text, n=1):
    ...
    """
    return len(re.findall(rf"(.)(?=\1{{{n}}})", text, flags=re.DOTALL))
...
def check_surrounding_chars(text, surrounding_chars):
    ...
    """
    surrounding = "".join(re.escape(char) for char in surrounding_chars)
    return len(
        re.findall(
            rf"((?<=[{surrounding}])(.)(?=[{surrounding}]))",
            text,
