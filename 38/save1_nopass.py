...
def get_movies(xml):
    ...
    root = get_tree(xml)
    return [movie.get('title') for movie in root.findall('movie')]


def get_movie_longest_runtime(xml):
    ...
    def _get_runtime(root):
        return {movie.get('runtime'): movie.get('title') for movie in root.findall('movie')}

    root = get_tree(xml)
    runtime_dict = _get_runtime(root)
    return runtime_dict[max(runtime_dict)]
