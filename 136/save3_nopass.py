...
def validate_bloodtype(bt):
    # First validate that a validate blood type was given
    if not isinstance(bt, (int, str, Bloodtype)):
        raise TypeError
    if isinstance(bt, str) and bt not in blood_type_text.keys():
        raise ValueError
    if isinstance(bt, int) and bt not in range(1,8):
        raise ValueError

    # Have a validate blood type, return bloodtype int
    # This is so we can use the _particular_antigen_comp helper function
    if isinstance(bt, int):
        return bt
    elif isinstance(bt, Bloodtype):
        return bt.value
    else:
        return blood_type_text[bt].value
...
def check_bt(donor, recipient):
    ...
    donor_int = validate_bloodtype(donor)
    recip_int = validate_bloodtype(recipient)

    # Calculate donor / recipient compatibility
    compat_tup = _particular_antigen_comp(donor_int, recip_int)

    # Return the result
    compat = True
    for comp in compat_tup:
        if comp < 0:
            compat = False
    return compat
