...
def test_first_row_is_correct(output):
    data = json.loads(output)
    assert type(data[0]) == dict
    assert list(data[0].keys()) == ["id", "first_name", "last_name", "email"]
    assert list(data[0].values()) == ["1", "Junie", "Kybert", "jkybert0@army.mil"]
    assert list(data[-1].values()) == [
        "10",
        "Wyn",
        "Serginson",
        "wserginson9@naver.com",
    ]
