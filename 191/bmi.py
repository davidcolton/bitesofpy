...
BodyMassIndex = namedtuple("BodyMassIndex", "person bmi")
...
def _calculate_bmi(person):
    person, height, mass = person.strip().split(",")
    return person, round(float(mass) / ((int(height) / 100) ** 2), 2)


def person_max_bmi(data=data):
    """Return (name, BMI float) of the character in data that
       has the highest BMI (rounded on 2 decimals)"""
    return max([_calculate_bmi(line) for line in data.splitlines()], key=lambda p: p[1])
