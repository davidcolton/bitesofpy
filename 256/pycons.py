...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    # PyCon = namedtuple("PyCon", "name city country start_date end_date url")
    for event in json_data:
        event_json = eval(event.text.strip())
        name = event_json["name"]
        city = event_json["location"]["address"]["addressLocality"]
        country = event_json["location"]["address"]["addressCountry"]
        start_date = parse(event_json["startDate"])
        end_date = parse(event_json["endDate"])
        url = event_json["url"]
        if name.lower().startswith("pycon"):
            pycon_list.append(PyCon(name, city, country, start_date, end_date, url))
...
def filter_pycons(
    ...
    return [
        event
        for event in pycons
        if event.start_date.year == year and get_continent(event.country) == continent
    ]
