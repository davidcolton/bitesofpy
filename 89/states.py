...
def get_longest_state(data):
    """Receives data, which can be the us_state_abbrev dict or the states
       ...
       of the string"""
    state_lengths = (
        [(state, len(state)) for state in data.keys()]
        if isinstance(data, dict)
        else [(state, len(state)) for state in data]
    )
    return sorted(state_lengths, key=lambda k: (-k[1]))[0][0]
...
def combine_state_names_and_abbreviations(
    ...
    """Get the first 10 state abbreviations ('AL', 'AK', 'AZ', ...) from
       ...
       alphabetical order"""
    first_ten = sorted(list(us_state_abbrev.values()))[:10]
    last_ten = sorted(states)[-10:]
