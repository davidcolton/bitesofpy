...
def filter_killed_mutants(mutpy_output: list = None) -> list:
    ...
    closing_pattern = re.compile(r"\[\d{1,}.\d+ s\]")

    filtered_output = []

    opening_index = [
        idx for idx, line in enumerate(mutpy_output) if opening_pattern.match(line)
    ]
    closing_index = [
        idx for idx, line in enumerate(mutpy_output) if closing_pattern.match(line)
    ]

    # List of tuples with the first line and last line indexes of each mutpy block
    blocks = list(zip(opening_index, closing_index))

    # Add the opening statements
    filtered_output.extend(mutpy_output[: blocks[0][0]])

    # Process each block
    for block in blocks:
        if "survived" in mutpy_output[block[1]]:
            filtered_output.extend(mutpy_output[block[0] : block[1] + 1])
        else:
            filtered_output.extend([mutpy_output[block[0]]])
            filtered_output.extend([mutpy_output[block[1]]])

    # Add the closing statements
    filtered_output.extend(mutpy_output[blocks[-1][1] + 1 :])
